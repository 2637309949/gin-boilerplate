...
func (s *Handler) QueryUserDB(ctx context.Context, session *gorm.DB, where *models.User, list *[]models.User, count ...*int32) error {
...
func (s *Handler) QueryUserDetailDB(ctx context.Context, session *gorm.DB, where *models.User, data *models.User) error {
	var err error
	var lst []models.User
	s.QueryUserDB(ctx, session, where, &lst)
	if err != nil {
		return err
	}
	if len(lst) == 0 {
...
func (s *Handler) InsertUserDB(ctx context.Context, session *gorm.DB, data *models.User) error {
...
func (s *Handler) UpdateUserDB(ctx context.Context, session *gorm.DB, data *models.User) error {
